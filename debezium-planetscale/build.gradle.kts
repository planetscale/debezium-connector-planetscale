...
// Dependency locking is now handled at the root project level
...
dependencies {
  ...
  planetscaleAdapter(project(":transforms"))
...
val transformVitess by tasks.registering(ByteBuddyTask::class) {
  ...
  dependsOn(tasks.compileKotlin, debeziumClasses, debeziumClassesPatched, ":transforms:jar")
  
  // TODO: Configure ByteBuddy transformations for GEOMETRY support after fixing basic build
  // For now, we'll disable transformations to get the basic multi-module build working
}
