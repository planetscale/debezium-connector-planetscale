...
| 3 | **`vitess.cells`**: set `VStreamFlags.cells` so vtgate serves the stream from tablets in the named cell(s) | S | Locality control for any multi-cell Vitess user. Adaptation: upstream version must be optional + registered in `CONFIG_DEFINITION` (our copy marks it required and never registers it) | [dbz#2547](https://github.com/debezium/dbz/issues/2547) → [PR #300](https://github.com/debezium/debezium-connector-vitess/pull/300) — open, in review (extended with `vitess.cell.preference` after review); docs: [debezium#7953](https://github.com/debezium/debezium/pull/7953) |
...
- [ ] PR 3 — `vitess.cells` (+ `vitess.cell.preference`) — PR open, in review; docs PR open
