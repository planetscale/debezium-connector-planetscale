...
| 3 | **`vitess.cells`**: set `VStreamFlags.cells` so vtgate serves the stream from tablets in the named cell(s) | S | Locality control for any multi-cell Vitess user. Adaptation: upstream version must be optional + registered in `CONFIG_DEFINITION` (our copy marks it required and never registers it) | [dbz#2547](https://github.com/debezium/dbz/issues/2547) → [PR #300](https://github.com/debezium/debezium-connector-vitess/pull/300) — **merged 2026-09-24** (extended with `vitess.cell.preference` after review); docs: [debezium#7953](https://github.com/debezium/debezium/pull/7953) — merged 2026-09-24 |
...
- [x] PR 3 — `vitess.cells` (+ `vitess.cell.preference`) — merged 2026-09-24, docs merged
